...
#include "esp32c3/rom/ets_sys.h"//esp32c3/rom/ets_sys.h

#include "common.h"
#include "dht11.h"

//refer to: https://gitee.com/vi-iot/esp32-board/blob/master/dht11/main/dht11.c
...
static bool IRAM_ATTR rmt_rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata, void *user_data)
{
  ...
  return high_task_wakeup == pdTRUE; // 返回是否唤醒了任何任务
}
...
int dht11Read(uint8_t dht11_pin,int *temprature, int *humidity)
{
  ...
  ets_delay_us(1000);//busy-waiting – it does not allow other tasks to run, it just burns CPU cycles.
  gpio_set_level(dht11_pin, 0);
  ets_delay_us(20000);//Note that this is busy-waiting – it does not allow other tasks to run, it just burns CPU cycles.
  ...
  if (xQueueReceive(rx_receive_queue, &rx_data, pdMS_TO_TICKS(1000)) == pdTRUE)
  {
    ...
    return parse_items(rx_data.received_symbols, rx_data.num_symbols, humidity, temprature);
  }
  return 0;
}

// 将RMT读取到的脉冲数据处理为温度和湿度(rmt_symbol_word_t称为RMT符号)
static int parse_items(rmt_symbol_word_t *item, int item_num, int *humidity, int *temprature)
{
  int i = 0;
